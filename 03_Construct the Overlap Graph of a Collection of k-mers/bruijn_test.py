...
def de_bruijn(k, text):
    ...
    for i in range(len(text) - k + 1):
        ...
        prefix = kmer[:-1]
        suffix = kmer[1:]
        G.add_edge(prefix, suffix) # declarer une arete permet de creer les noeuds automatiquement

    # affichage liste d'adjacence
    for node in sorted(G.nodes()):
        successors = sorted(G.neighbors(node)) # on recup les neouds suivants, par ordre alpha
